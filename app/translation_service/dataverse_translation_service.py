...
class DataverseTranslationService(TranslationService):
    ...
    def prepare_and_send_to_drs(self, package_dir, supplemental_deposit_data, depositing_application, testing = False):
        ...
        self.logger.debug("Supplemental deposit data: %s", supplemental_deposit_data)
        ...
        try:
            ...
            with open(drs_config_path, 'r', encoding='UTF-8') as file:
                ...
                try:
                    ...
                    self.logger.error("Missing a key in " + drs_config_path +" file: " + str(err))

        except FileNotFoundError:
            self.logger.error("drsConfig.txt does not exist for path: "+ drs_config_path)
